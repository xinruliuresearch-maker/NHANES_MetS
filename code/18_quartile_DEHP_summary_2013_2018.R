# ============================================================
# NHANES 2013-2018
# 18_quartile_DEHP_summary_2013_2018.R
# Quartile dose-response for ΣDEHP and metabolism profile
# ============================================================
...
exposure_map <- tibble::tribble(
  ...
  "ln_Sigma_DEHP", "ln(ΣDEHP)", TRUE,
  "pct_oxidative", "%Oxidative metabolites", FALSE,
...
run_quartile_model <- function(outcome, outcome_label, is_log_outcome,
  ...
  if (length(unique(d$exposure_q)) < 4) return(tibble())
  ...
  if (is.null(fit_cat)) return(tibble())
...
write_csv(
  quartile_results,
  file.path(result_dir, "DEHP_summary_quartile_2013_2018.csv")
)
...
write_xlsx(
  ...
  file.path(result_dir, "DEHP_summary_quartile_2013_2018.xlsx")
)
...
p <- ggplot(
  plot_df,
  aes(x = quartile, y = effect, group = exposure_label)
) +
  ...
  labs(
    title = "Quartile dose-response of ΣDEHP and DEHP metabolic profile with metabolic markers",
...
ggsave(
  file.path(fig_dir, "DEHP_summary_quartile_2013_2018.png"),
  p,
  width = 12,
  height = 6,
  dpi = 300
...
ggsave(
  file.path(fig_dir, "DEHP_summary_quartile_2013_2018.pdf"),
  p,
  width = 12,
  height = 6
)

cat("ΣDEHP 与 DEHP 代谢模式四分位剂量反应分析完成。\n")